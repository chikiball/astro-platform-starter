...
.container{max-width:100%;margin:0 auto;background:rgba(26,26,46,0.95);min-height:100vh;display:flex;flex-direction:column}
.header{background:linear-gradient(135deg,rgba(45,125,110,0.8),rgba(212,175,55,0.1));padding:12px 20px;text-align:center;border-bottom:2px solid #d4af37}
.header h1{color:#d4af37;font-size:clamp(1em,4vw,1.8em);text-shadow:2px 2px 4px rgba(0,0,0,0.5);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
#tab-1.active{display:flex;flex-direction:column;height:calc(100vh - var(--header-h, 100px))}
...
.tree-toolbar-row{display:flex;gap:6px;align-items:center;flex-wrap:nowrap;overflow-x:auto;padding-bottom:4px;scrollbar-width:none}
.tree-toolbar-row::-webkit-scrollbar{display:none}
.tree-toolbar-row button{flex-shrink:0}
...
.tree-wrap{overflow:hidden;background:rgba(0,0,0,0.2);border:1px solid rgba(45,125,110,0.3);border-radius:8px;flex:1;min-height:0;position:relative;cursor:grab;touch-action:none;user-select:none;-webkit-user-select:none}
...
.ftree .couple{transition:opacity 0.25s ease,filter 0.25s ease}
...
.tip{position:fixed;background:#0a0a14;color:#e0e0e0;padding:12px 16px;border-radius:6px;font-size:0.8em;white-space:normal;overflow-wrap:break-word;word-break:normal;min-width:220px;max-width:min(360px,90vw);z-index:100000;border:1px solid #d4af37;box-shadow:0 8px 30px rgba(0,0,0,1);opacity:0;pointer-events:none;transition:opacity 0.15s;text-align:left;line-height:1.5}
...
<div class="container">
  ...
  <div id="tab-1" class="tab-panel">
    ...
    <div id="floating-tip" class="tip"></div>
  </div>
...
function tipHTML(code){
  ...
  if(S[code] && S[code].of && P[S[code].of]){
    h+='<div class="trow"><span class="tlbl">Spouse of: </span>'+esc(P[S[code].of].name)+'</div>';
  }
...
function renderBranch(pCode){
  ...
  let coupleHTML = '<div class="couple'+(hit?' search-hit':'')+'" data-code="'+pCode+'">';
...
// ── Fisheye depth effect (layout-safe: no transform on individual nodes) ─────
(function(){
  ...
  function applyFisheye(){
    ...
    const maxD = Math.hypot(wRect.width, wRect.height) / 2;

    document.querySelectorAll('.ftree .couple').forEach(el=>{
      const r = el.getBoundingClientRect();
      // Skip elements with zero size (not rendered / off-screen)
      if(r.width === 0 && r.height === 0) return;
      const ex = r.left + r.width  / 2;
      const ey = r.top  + r.height / 2;
      const d  = Math.hypot(ex - cx, ey - cy);
      const t  = Math.min(d / (maxD * 1.2), 1);
      // opacity: 1.0 at centre → 0.55 at far edge
      const o  = 1 - t * 0.45;
      // brightness: 100% at centre → 60% at far edge (dims without hiding)
      const b  = 1 - t * 0.4;
  ...
  const obs = new MutationObserver(applyFisheye);
  obs.observe(inner, { attributes: true, attributeFilter: ['style'] });

  wrap.addEventListener('scroll', applyFisheye, { passive: true });
  window.addEventListener('resize', applyFisheye, { passive: true });

  setTimeout(applyFisheye, 200);
})();

// ── Measure header+tabs height so tree panel fills remaining screen ──────────
(function(){
  function setHeaderVar(){
    const header = document.querySelector('.header');
    const tabs   = document.querySelector('.tabs');
    const h = (header ? header.offsetHeight : 0) + (tabs ? tabs.offsetHeight : 0);
...
// ── Hover + Click/tap tooltip with smart positioning ─────────────────────────
// closeAllTips is now replaced by closeTip() above

// ── Single floating tooltip outside the transform ────────────────────────────
...
function showTip(code, px, py){
  const html = tipHTML(code);
  if(!html){ closeTip(); return; }
  floatingTip.innerHTML = html;

  // Measure off-screen
  floatingTip.style.left = '-9999px';
  floatingTip.style.top  = '0';
  floatingTip.classList.add('visible');
  const tw = floatingTip.offsetWidth;
  const th = floatingTip.offsetHeight;
  floatingTip.classList.remove('visible');
  ...
  // Prefer above pointer, centred
  let left = px - tw / 2;
  let top  = py - th - pad;

  // No room above → show below
  if(top < pad) top = py + pad;
...
function getCoupleCode(couple){
  // Read the person code from the couple's tip div data, or find it from the DOM
  const tipDiv = couple.querySelector('.tip');
  if(tipDiv && tipDiv.dataset.code) return tipDiv.dataset.code;
  return null;
}

// Click/tap
document.addEventListener('click', function(e){
  if(e.target.closest('.tbtn')) return; // don't interfere with toggle buttons
  const couple = e.target.closest('.couple');
  if(couple){
    const code = couple.dataset.code;
    if(!code){ closeTip(); return; }
    if(tipPinned && floatingTip.dataset.code === code){
      closeTip();
    } else {
      showTip(code, e.clientX, e.clientY);
      floatingTip.dataset.code = code;
      tipPinned = true;
    }
  } else {
    closeTip();
